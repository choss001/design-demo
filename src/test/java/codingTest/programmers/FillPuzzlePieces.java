...
//        printTurnedPuzzle();
    ...
    private void correspondBlockFind(int[][] game_board, boolean[] turnedArrayUsedCheck, int y, int x, boolean[][] blankUsedCheck){
        for(int i =y; i < game_board.length; i++){
            for(int j = x; j < game_board[0].length; j++){
                ...
                if(game_board[i][j] == 0 && !blankUsedCheck[i][j]){
                    ...
                    emptyArray.add(new int[]{j, i});
                    boolean[][] usedCheckEmpty = new boolean[game_board.length][game_board[0].length];
                    usedCheckEmpty[i][j] = true;
                    List<int[]> valueResult = getGameEmptyBlock(j, i, game_board, emptyArray, usedCheckEmpty);
    ...
    private int[] getMaxAndMinValue(List<int[]> blanksBlock){
        ...
        if(xList.size() == 1){
            ...
            returnValue[1] = xList.get(0);
            returnValue[2] = yList.get(0);
            returnValue[3] = yList.get(0);
        }else{
            returnValue[0] = xList.get(0);
